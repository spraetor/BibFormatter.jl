using BibtexFormatter: BibliographyStyles,format
using Test
...
for style in (BibliographyStyles.abbrv,) #instances(BibliographyStyles.T)
  println(style)
...
if false
  ...
  for style in instances(BibliographyStyles.T)
    ...
    open(texFilename,"w") do texFile
      ...
      println(texFile, raw"\nocite{*}")
      println(texFile, raw"\bibliographystyle{" * string(style) * "}")
